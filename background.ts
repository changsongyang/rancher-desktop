...
Electron.app.whenReady().then(async() => {
  try {
    ...
    if (commandLineArgs.length) {
      try {
        cfg = updateFromCommandLine(cfg, settings.lockedSettings, commandLineArgs);
...
ipcMainProxy.on('get-locked-fields', () => {
  window.send('locked-fields-read', settings.lockedSettings);
});
...
class BackgroundCommandWorker implements CommandWorkerInterface {
  ...
  protected async validateSettings(existingSettings: settings.Settings, newSettings: RecursivePartial<settings.Settings>) {
    ...
    return this.settingsValidator.validateSettings(existingSettings, newSettings, settings.lockedSettings);
  }
